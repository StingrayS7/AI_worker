...
В качестве улучшения нейро-сотрудника можно проводить пособработку запроса через LLMLingua. Данная постобработка показывает хорошую эффективность, но требует больше ресуросв для работы (память GPU)

В T4 не запускается из-за нехватки памяти GPU, поэтому возможности протестировать не было.
"""
